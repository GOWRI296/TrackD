...
export default function ProductivityChart({ data }: ProductivityChartProps) {
  ...
  const latest = chartData[lastIndex]?.productivity_score ?? 0;
  const previous = chartData[lastIndex - 1]?.productivity_score ?? latest;
  ...
  return (
    <div className="card chart-card mt-8" style={{ minHeight: 460 }}>
      ...
      >
        <div>
          <h2 className="text-2xl font-bold">Productivity Trend</h2>
          <p style={{ color: "#86868b", fontSize: 14, marginTop: 4 }}>
            Daily score, curved against your real baseline
          </p>
        </div>
